...
export default function Home() {
  ...
  const [customWorkoutValues, setCustomWorkoutValues] = useState<{
    targetReps: number;
    targetWeight: number;
  } | null>(null);
  ...
  useEffect(() => {
    if (customWorkoutValues) {
      setReps(customWorkoutValues.targetReps);
      setRepsInput(String(customWorkoutValues.targetReps));
      setWeight(customWorkoutValues.targetWeight);
      setWeightInput(String(customWorkoutValues.targetWeight));
    }
  }, [customWorkoutValues]);
  ...
  const handleTargetValuesChange = useCallback((reps: number, weight: number) => {
    setCustomWorkoutValues({ targetReps: reps, targetWeight: weight });
  }, []);
          ...
          <TabsContent value="workout" className="mt-6 flex flex-col h-full overflow-auto">
            <div className="flex-1 overflow-auto">
              <CustomWorkouts 
                ...
                onTargetValuesChange={handleTargetValuesChange}
              />
              
              {selectedExercise && workoutData.length > 0 && (
